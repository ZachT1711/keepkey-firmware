...
void signing_init(const SignTx *msg, const CoinType *_coin, const HDNode *_root)
{
	...
	tx_init(&to, inputs_count, outputs_count, version, lock_time, expiry, 0, curve->hasher_sign, overwintered, version_group_id);
...
static bool signing_check_input(TxInputType *txinput) {
	...
	if (txinput->has_multisig && !multisig_fp_mismatch) {
		...
		if (cryptoMultisigFingerprint(&txinput->multisig, h) == 0) {
			fsm_sendFailure(FailureType_Failure_Other, _("Error computing multisig fingerprint"));
			signing_abort();
			return false;
		}
		if (multisig_fp_set) {
	...
	if (coin->decred) {
		if (txinput->decred_script_version > 0) {
			fsm_sendFailure(FailureType_Failure_SyntaxError, _("Decred v1+ scripts are not supported"));
			signing_abort();
			return false;
		}
...
static bool signing_check_prevtx_hash(void) {
	...
	if (memcmp(hash, input.prev_hash.bytes, 32) != 0) {
		fsm_sendFailure(FailureType_Failure_Other, _("Encountered invalid prevhash"));
		signing_abort();
		return false;
	}
...
static bool signing_check_output(TxOutputType *txoutput) {
	...
	if (co <= TXOUT_COMPILE_ERROR) {
		...
		fsm_sendFailure(FailureType_Failure_SyntaxError, _("Failed to compile output"));
		signing_abort();
		return false;
	}
	if (coin->decred) {
...
static bool signing_sign_hash(TxInputType *txinput, const uint8_t* private_key, const uint8_t *public_key, const uint8_t *hash) {
	...
	if (txinput->has_multisig) {
		...
		if (pubkey_idx < 0) {
			fsm_sendFailure(FailureType_Failure_SyntaxError, _("Pubkey not found in multisig script"));
			signing_abort();
			return false;
		}
	...
	switch (signing_stage) {
		case STAGE_REQUEST_1_INPUT:
			...
			if (tx->inputs[0].script_type == InputScriptType_SPENDMULTISIG
				...
				if (idx1 > 0 && to.is_segwit == true) {
					fsm_sendFailure(FailureType_Failure_SyntaxError, _("Mixing segwit and non-segwit inputs is not allowed"));
					signing_abort();
					return;
				}
#endif
				...
				if (coin->force_bip143 || overwintered) {
					if (!tx->inputs[0].has_amount) {
						fsm_sendFailure(FailureType_Failure_SyntaxError, _("Expected input with amount"));
						signing_abort();
						return;
					}
				...
				if (coin->decred) {
					fsm_sendFailure(FailureType_Failure_SyntaxError, _("Decred does not support Segwit"));
					signing_abort();
					return;
				}
				if (!coin->has_segwit || !coin->segwit) {
				...
				if (!tx->inputs[0].has_amount) {
					fsm_sendFailure(FailureType_Failure_SyntaxError, _("Segwit input without amount"));
					signing_abort();
					return;
				}
				if (to_spend + tx->inputs[0].amount < to_spend) {
		...
		case STAGE_REQUEST_2_PREV_META:
			if (tx->outputs_cnt <= input.prev_index) {
				fsm_sendFailure(FailureType_Failure_SyntaxError, _("Not enough outputs in previous transaction."));
				signing_abort();
				return;
			}
			...
			tx_init(&tp, tx->inputs_cnt, tx->outputs_cnt, tx->version, tx->lock_time, tx->expiry, tx->extra_data_len, curve->hasher_sign, overwintered, version_group_id);
			if (coin->decred) {
		...
		case STAGE_REQUEST_2_PREV_OUTPUT:
			...
			if (idx2 == input.prev_index) {
				...
				if (coin->decred && tx->bin_outputs[0].decred_script_version > 0) {
					fsm_sendFailure(FailureType_Failure_SyntaxError, _("Decred script version does not match previous output"));
					signing_abort();
					return;
				}
		...
		case STAGE_REQUEST_2_PREV_EXTRADATA:
			if (!tx_serialize_extra_data_hash(&tp, tx->extra_data.bytes, tx->extra_data.size)) {
				fsm_sendFailure(FailureType_Failure_Other, _("Failed to serialize extra data"));
				signing_abort();
				return;
			}
				...
				if (overwintered) {
					switch (version) {
						...
						case 4:
							...
							break;
						default:
							fsm_sendFailure(FailureType_Failure_SyntaxError, _("Unsupported version for overwintered transaction"));
							signing_abort();
							return;
					}
				} else {
